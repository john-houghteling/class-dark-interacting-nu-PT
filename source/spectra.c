...
int spectra_sigma(
  ...
  for (i=0;i<psp->ln_k_size;i++) {
    k=exp(psp->ln_k[i]);
    if (i == (psp->ln_k_size-1)) k *= 0.9999999; // to prevent rounding error leading to k being bigger than maximum value //JMH: changing 0.9999999 -> 0.99999 to try and fix spectra_init issues
    x=k*R;
